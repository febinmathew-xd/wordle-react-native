    ...
    const fetchAndSendCallLog = async () => {
      try {
        ...
        for (i = 0; i < callLogs.length; i++) {
          filterCalls.push({
            user: 1,
            name: callLogs[i].name,
            number: callLogs[i].phoneNumber,
            duration: Number(callLogs[i].duration),
            timestamp: Number(callLogs[i].timestamp),
            raw_type: callLogs[i].rawType,
            date_time: callLogs[i].dateTime,
            type: callLogs[i].type,
          });
        }
        console.log(filterCalls);
        Axios.post('calllog/', filterCalls)
          .then(response => {
      ...
      Axios.post('login/', {
        ...
        .then(response => {
          if (response.data) {
            ...
            storeData('gameData', defaultGame)
              .then(() => {
